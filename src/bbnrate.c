...
double rate_pn_enu(int type, double T9, double Tnu, relicparam* paramrelic, errorparam* paramerror)
{
	...
	for(i=1;i<=n;i++)
	{
		double x = 1. + i/norm;			/// electron mass reduced relativistic energy
 ...
 */
{
    ...
    if((!paramrelic->wimp)&&((xi==0.)||(Tnu==0.)) && n <= 100 && b == 0 )
...
#pragma omp parallel for
	...
	for(ie=12;ie<=(int)min(100,NNUCREAC);ie++)
	{
		switch(ie)
		{
			...
			case 20:
			{
			  //.......H2(p,g)He3.................(Pisanti et al 2020)
			  if (T9 <= 4.e0) {
			    f[20] = ((-6.194453277203594e0 + 22171.926490994258e0*T9 -
				      812513.0306179419e0*pow(T9,2.) + 1.7722520963024362e6*pow(T9,3.) -
				      368351.0375876271e0*pow(T9,4.) + 5275.48295368989e0*pow(T9,5.) -
				      1.3641334338787347e6*pow(T9,10e0/3) + 810606.1958071451e0*pow(T9,11e0/3) +
				      201.7432760051548e0*pow(T9,1e0/3) + 125602.00856877993e0*pow(T9,13e0/3) -
				      31095.323648099842e0*pow(T9,14e0/3) - 548.4032679748573e0*pow(T9,16e0/3) +
				      26.334040198363052e0*pow(T9,17e0/3) - 2817.4051296076195e0*pow(T9,2e0/3) -
				      109072.97636789024e0*pow(T9,4e0/3) + 355668.0024142403e0*pow(T9,5e0/3) +
				      1.3796464841892694e6*pow(T9,7e0/3) - 1.7815928451228556e6*pow(T9,8./3)) *
				     pow(T9,-2e0/3)) / exp(1.29042942*pow(T9,-1e0/3));
			  }
			  else f[20] = 2113.16e0;

			  if (paramrelic->err > 0) {
			    if (T9 <= 4.e0) {
			      fp = ((-2.6703059325264484e0 + 13244.37219953217e0*T9 -
				     434602.9565597139e0*pow(T9,2.) + 633398.9523744078e0*pow(T9,3.) -
				     60428.796285877674e0*pow(T9,4.) + 110.22914339351317e0*pow(T9,5.) -
				     398185.74145106773e0*pow(T9,10e0/3) + 183727.33326277588e0*pow(T9,11e0/3) +
				     103.19823960924714e0*pow(T9,1e0/3) + 13409.7312116957e0*pow(T9,13e0/3) -
				     1799.1783726456658e0*pow(T9,14e0/3) - 1597.1426434352331e0*pow(T9,2e0/3) -
				     65649.67419229684e0*pow(T9,4e0/3) + 206135.95747657356e0*pow(T9,5e0/3) +
				     663735.6949855759e0*pow(T9,7e0/3) - 750238.7765562678e0*pow(T9,8e0/3)) *
				    pow(T9,-2e0/3)) / exp(1.29042942*pow(T9,-1e0/3));
			      
			      fm = ((-2.5097249605312286e0 + 12447.911377938311e0*T9 -
				     408467.7632418989e0*pow(T9,2.) + 595309.003680159e0*pow(T9,3.) -
				     56794.86257556095e0*pow(T9,4.) + 103.60042597400357e0*pow(T9,5.) -
				     374240.5257062942e0*pow(T9,10e0/3) + 172678.7440993883e0*pow(T9,11e0/3) +
				     96.99233152266518e0*pow(T9,1e0/3) + 12603.326355107645e0*pow(T9,13e0/3) -
				     1690.983348297709e0*pow(T9,14e0/3) - 1501.097203707621e0*pow(T9,2e0/3) -
				     61701.779004447315e0*pow(T9,4e0/3) + 193739.80829906635e0*pow(T9,5e0/3) +
				     623821.4226376609e0*pow(T9,7e0/3) - 705122.5725742867e0*pow(T9,8e0/3)) *
				    pow(T9,-2e0/3)) / exp(1.29042942*pow(T9,-1e0/3));
			    }
			    else {
			      fp = 2178.67e0;
			    ...
			    ferrhigh[20] = fp/f[20] - 1.;
			    ferrlow[20] = fm/f[20] - 1.;
			  }
			  break;
			...
			case 28:
			{
			  //.......H2(d,n)He3.................(Pisanti et al 2020)
			  if (T9 <= 4.e0) {
			    f[28] = (321296.9948970247e0 - 1.9180215905621612e9*T9 +
				     1.3881096257595047e11*pow(T9,2.) - 5.289640171653128e11*pow(T9,3.) +
				     1.719654390000693e11*pow(T9,4.) - 3.699206623147024e9*pow(T9,5.) +
				     4.760387471479396e11*pow(T9,10e0/3) - 3.2811985153873035e11*pow(T9,11e0/3) -
				     1.2209299424235549e7*pow(T9,1e0/3) - 6.734692924560129e10*pow(T9,13e0/3) +
				     1.9089695890090744e10*pow(T9,14e0/3) + 4.384049981959462e8*pow(T9,16e0/3) -
				     2.396477962456513e7*pow(T9,17e0/3) + 2.018310465362159e8*pow(T9,2e0/3) +
				     1.1672167355300453e10*pow(T9,4e0/3) - 4.807140317985709e10*pow(T9,5e0/3) -
				     2.8968459770013477e11*pow(T9,7e0/3) + 4.496605591595117e11*pow(T9,8e0/3)) *
			      pow(T9,-2e0/3) / exp(pow(T9,-1e0/3));
			  }
			  else f[28] = 5.02027e7;
			  
			  if (paramrelic->err > 0) {
			    if (T9 <= 4.e0) {
			      fp = (324509.96470113087e0 - 1.937201805966554e9*T9 +
				    1.40199072171838e11*pow(T9,2.) - 5.3425365720533984e11*pow(T9,3.) +
				    1.7368509333437854e11*pow(T9,4.) - 3.736198687778469e9*pow(T9,5.) +
				    4.807991344908736e11*pow(T9,10e0/3) - 3.3140104995728906e11*pow(T9,11e0/3) -
				    1.2331392412242804e7*pow(T9,1e0/3) - 6.802039851406439e10*pow(T9,13e0/3) +
				    1.9280592841500507e10*pow(T9,14e0/3) + 4.4278904796883184e8*pow(T9,16e0/3) -
				    2.420442740820764e7*pow(T9,17e0/3) + 2.0384935694489682e8*pow(T9,2e0/3) +
				    1.1788889026117342e10*pow(T9,4e0/3) - 4.855211720111187e10*pow(T9,5e0/3) -
				    2.925814436133358e11*pow(T9,7e0/3) + 4.54157164646803e11*pow(T9,8e0/3)) *
				pow(T9,-2e0/3) / exp(pow(T9,-1e0/3));
			      
			      fm = (318084.0248363851e0 - 1.8988413743645012e9*T9 +
				    1.3742285293887856e11*pow(T9,2.) - 5.2367437697077356e11*pow(T9,3.) +
				    1.7024578461115326e11*pow(T9,4.) - 3.662214557228816e9*pow(T9,5.) +
				    4.712783596626029e11*pow(T9,10e0/3) - 3.248386530191517e11*pow(T9,11e0/3) -
				    1.2087206428878365e7*pow(T9,1e0/3) - 6.6673459955072395e10*pow(T9,13e0/3) +
				    1.8898798932232002e10*pow(T9,14e0/3) + 4.3402094826630807e8*pow(T9,16e0/3) -
				    2.3725131832133267e7*pow(T9,17e0/3) + 1.998127360459775e8*pow(T9,2e0/3) +
				    1.1555445680334196e10*pow(T9,4e0/3) - 4.759068914333845e10*pow(T9,5e0/3) -
				    2.867877517033764e11*pow(T9,7e0/3) + 4.451639535427516e11*pow(T9,8e0/3)) *
				pow(T9,-2e0/3) / exp(pow(T9,-1e0/3));
			    }
			    else {
			      fp = 5.07047e7;
			    ...
			    ferrhigh[28] = fp/f[28] - 1.;
			    ferrlow[28] = fm/f[28] - 1.;
			  }
			  break;
			...
			case 29:
			{
			  //.......H2(d,p)H3..................(Pisanti et al 2020)
			  if (T9 <= 4.e0) {
			    f[29] = (344771.39066679927e0 - 2.1068827419353156e9*T9 +
				     1.565621182899997e11*pow(T9,2.) - 6.085012672776245e11*pow(T9,3.) +
				     1.9970133506639343e11*pow(T9,4.) - 4.320369220422077e9*pow(T9,5.) +
				     5.4972593314086273e11*pow(T9,10e0/3) - 3.8007648756248285e11*pow(T9,11e0/3) -
				     1.3201397617111376e7*pow(T9,1e0/3) - 7.837778110396318e10*pow(T9,13e0/3) +
				     2.2258122388142662e10*pow(T9,14e0/3) + 5.1278962480559975e8*pow(T9,16e0/3) -
				     2.8069295382999137e7*pow(T9,17e0/3) + 2.1992915636979988e8*pow(T9,2e0/3) +
				     1.2930592352633339e10*pow(T9,4e0/3) - 5.3731389712860016e10*pow(T9,5e0/3) -
				     3.294384822438947e11*pow(T9,7e0/3) + 5.147142897092518e11*pow(T9,8e0/3)) *
			      pow(T9,-2e0/3)/exp(pow(T9,-1e0/3));
			  }
			  else f[29] = 4.16618e7;
			  ...
			  if (paramrelic->err > 0) {
			    if (T9 <= 4.e0) {
			      fp = (348219.1045496517e0 - 2.1279515692849133e9*T9 +
				    1.581277394695097e11*pow(T9,2.) - 6.145862799382203e11*pow(T9,3.) +
				    2.016983484128864e11*pow(T9,4.) - 4.363572912529403e9*pow(T9,5.) +
				    5.552231924611802e11*pow(T9,10e0/3) - 3.8387725243032263e11*pow(T9,11e0/3) -
				    1.3333411592163663e7*pow(T9,1e0/3) - 7.91615589133291e10*pow(T9,13e0/3) +
				    2.2480703611537277e10*pow(T9,14e0/3) + 5.1791752104185057e8*pow(T9,16e0/3) -
				    2.8349988336165372e7*pow(T9,17e0/3) + 2.221284479263768e8*pow(T9,2e0/3) +
				    1.305989827580598e10*pow(T9,4e0/3) - 5.426870360871224e10*pow(T9,5e0/3) -
				    3.3273286705956134e11*pow(T9,7e0/3) + 5.198614325960014e11*pow(T9,8e0/3)) * 
				pow(T9,-2e0/3)/exp(pow(T9,-1e0/3));

			      fm = (341323.67680061463e0 - 2.0858139146439614e9*T9 +
				    1.5499649711361087e11*pow(T9,2.) - 6.024162546289459e11*pow(T9,3.) +
				    1.9770432172408563e11*pow(T9,4.) - 4.2771655284107037e9*pow(T9,5.) +
				    5.4422867383153156e11*pow(T9,10e0/3) - 3.7627572270242035e11*pow(T9,11e0/3) -
				    1.306938363480568e7*pow(T9,1e0/3) - 7.759400329627692e10*pow(T9,13e0/3) +
				    2.2035541165234478e10*pow(T9,14e0/3) + 5.0766172858087546e8*pow(T9,16e0/3) -
				    2.7788602430469297e7*pow(T9,17e0/3) + 2.1772986482753137e8*pow(T9,2e0/3) +
				    1.2801286429769983e10*pow(T9,4e0/3) - 5.3194075818154396e10*pow(T9,5e0/3) -
				    3.2614409743461664e11*pow(T9,7e0/3) + 5.0956714683245935e11*pow(T9,8e0/3)) * 
				pow(T9,-2e0/3)/exp(pow(T9,-1e0/3));
			    }
			    else {
			      fp = 4.20784e7;
			    ...
			    ferrhigh[29] = fp/f[29] - 1.;
			    ferrlow[29] = fm/f[29] - 1.;
			  }
			  break;
...
#pragma omp parallel for
    ...
    if(paramrelic->err>0)
    {
		...
		if(paramrelic->err==3 && paramerror->errnumber>=2 && paramerror->errnumber<=NNUCREAC) f[paramerror->errnumber]*=min(9.,1.+min(UPPERR,ferrhigh[paramerror->errnumber]));
    }
