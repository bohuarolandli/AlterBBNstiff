...
/* Routine computing the time-derivatives of T, phie, h_eta, a, rho_phi and Y[] */
{
	...
	double rhod = dark_density_a(a,paramrelic);
	double Pd = dark_density_pressure_a(a,paramrelic);
	double drhod_dT=0.;
...
//   reac# type n1 n2 n3 n4 n5 n6 rev q
    ...
    double Ytmin =1.e-30;
    //double a = 8.e-11;                // Initial scale factor
    double a = 1.e0;                    // a/a_i
   ...
   //-----------------------------------------------------------------
    ...
    rhod = dark_density_a(1.,paramrelic);
...
#if defined(_OPENMP)	
#pragma omp parallel for private(ie) //shared(niter,paramrelic,ratioH_all,optfail)
#endif
