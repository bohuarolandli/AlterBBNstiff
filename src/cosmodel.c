...
void Init_cosmomodel(struct relicparam* paramrelic)
{
    ...
    paramrelic->Nnu=3.044;          // Number of SM neutrinos, e+- reheating included
    paramrelic->dNnu=0.;            // Number of extra neutrino species (e.g. sterile neutrinos)
    paramrelic->dd0_rad=0.;           // Number of extra radiation species which redshift as a^{-4}
    paramrelic->life_neutron = 878.4; // Neutron lifetime (PDG2022)
    paramrelic->life_neutron_error = 0.5; // Neutron lifetime uncertainty (PDG2022)    
...
void Init_cosmomodel_param(double eta, double Nnu, double dNnu, double dd0_rad, double life_neutron, double life_neutron_error, double xinu1,
...
void Init_dark_density(double dd0, double ndd, double T_end, double dd0_rad, struct relicparam* paramrelic)
{
...
void Init_scalarfield(double rhotilde_phi_Tmax, double Tmax, double T_RH, double eta_phi, double n_phi, struct relicparam* paramrelic)
{
	...
	Init_dark_density(0.,0.,0.,0.,paramrelic);
...
/*---------    dark density and pressure as functions of scale factor   ----------*/


double dark_density_a(double a, struct relicparam* paramrelic)
{
	if(paramrelic->energy_model==1)
	{
		//if(paramrelic->dd0==0.) return 0.;
		...
		double rho_photon_i = pi*pi/15. * pow(Ti, 4.);
				
		return (paramrelic->dd0*pow(a,-paramrelic->ndd) + paramrelic->dd0_rad*pow(a,-4.))*rho_photon_i;
	}
...
double dark_density_pressure_a(double a, struct relicparam* paramrelic)
{
	...
	if(paramrelic->energy_model==1)
	{
        ...
        double Ti = paramrelic->Tinit * K_to_eV;
		double rho_photon_i = pi*pi/15. * pow(Ti, 4.);
        double w = paramrelic->ndd/3. - 1.;
	
		return (paramrelic->dd0*pow(a,-paramrelic->ndd)*w + paramrelic->dd0_rad*pow(a,-4.)/3.)*rho_photon_i;
	}
